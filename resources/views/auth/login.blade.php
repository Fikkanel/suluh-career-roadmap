<x-layouts.auth title="Masuk" subtitle="Selamat datang kembali di Suluh">
    @if($errors->any())
    ...
    <form method="POST" action="{{ route('login.post') }}" class="flex flex-col gap-4">
        ...
        <label class="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" name="remember" class="accent-green-700">
            Ingat saya
        </label>
        <button type="submit" class="btn btn-primary w-full justify-center mt-1">Masuk</button>
    </form>

    <div class="flex items-center gap-3 mt-5">
        <hr style="flex:1;border:none;border-top:1px solid var(--border);">
        <span class="text-xs" style="color:var(--muted);white-space:nowrap;">atau lanjutkan dengan</span>
        <hr style="flex:1;border:none;border-top:1px solid var(--border);">
    </div>
